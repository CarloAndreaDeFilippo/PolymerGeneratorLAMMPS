...
def writeInputFileLAMMPS(partsys: ParticleSystem):

    with open(f"{partsys.par["inputFile"]}", "w+") as f:
        ...
        if int(partsys.par['restart']) == 1:
            ...
            f.write(f"read_data {partsys.par["atomFile"]}")

            if partsys.par['npatch'] > 0 and partsys.par['crosslink'] == True:
        ...
        f.write(f"pair_style lj/cut {minPositionLJ(partsys.sigma_bead)}\n")
        f.write(f"pair_modify shift yes\n")

        #LJ interaction: type1 type2 epsilon sigma cutoff

        #Beads-beads interaction
        f.write(f"pair_coeff {partsys.beadType} {partsys.beadType} {partsys.par['eps']} {partsys.sigma_bead} {minPositionLJ(partsys.sigma_bead)}\n")   #! WCA

        if partsys.par['nsolvent'] > 0:
            #Bead-solvent interaction
            sigma_bead_solv = 0.5 * (partsys.sigma_bead + partsys.sigma_solvent)
            f.write(f"pair_coeff {partsys.beadType} {partsys.solventType} {partsys.par['eps']} {sigma_bead_solv} {minPositionLJ(sigma_bead_solv)}\n")   #! WCA
            
            #Solvent-solvent interaction
            f.write(f"pair_coeff {partsys.solventType} {partsys.solventType} {partsys.par['eps_ss']} {partsys.sigma_solvent} {maxDistanceInteractionLJ(partsys.sigma_solvent)}\n") #! LJ cut

        if partsys.par['npatch'] > 0:
            #Bead-patch interaction
            sigma_bead_patch = 0.5 * (partsys.sigma_bead + partsys.sigma_patch)
            f.write(f"pair_coeff {partsys.beadType} {partsys.patchType} {partsys.par['eps']} {sigma_bead_patch} {minPositionLJ(sigma_bead_patch)}\n")   #! WCA
            
            #Patch-patch interaction
            f.write(f"pair_coeff {partsys.patchType} {partsys.patchType} {partsys.par['eps']} {partsys.sigma_patch} {minPositionLJ(partsys.sigma_patch)}\n")   #! WCA

            if partsys.par['nsolvent'] > 0:
                #Patch-solvent
                sigma_patch_solv = 0.5 * (partsys.sigma_patch + partsys.sigma_solvent)
                f.write(f"pair_coeff {partsys.patchType} {partsys.solventType} {partsys.par['eps']} {sigma_patch_solv} {minPositionLJ(sigma_patch_solv)}\n")   #! WCA


        if partsys.par['ncolloids'] > 0:
            #Bead-colloid interaction
            sigma_bead_coll = 0.5 * (partsys.sigma_bead + partsys.sigma_colloid)
            f.write(f"pair_coeff {partsys.beadType} {partsys.colloidType} {partsys.par['eps_bc']} {sigma_bead_coll} {maxDistanceInteractionLJ(sigma_bead_coll)}\n")   #! LJ cut
            
            #Colloid-colloid interaction
            f.write(f"pair_coeff {partsys.colloidType} {partsys.colloidType} {partsys.par['eps']} {partsys.sigma_colloid} {minPositionLJ(partsys.sigma_colloid)}\n")    #! WCA

            if partsys.par['nsolvent'] > 0:
                #Colloid-solvent interaction
                sigma_coll_solv = 0.5 * (partsys.sigma_colloid + partsys.sigma_solvent)
                f.write(f"pair_coeff {partsys.colloidType} {partsys.solventType} {partsys.par['eps_cs']} {sigma_coll_solv} {maxDistanceInteractionLJ(sigma_coll_solv)}\n")   #! LJ cut
        
            if partsys.par['npatch'] > 0:
                #Colloid-patch interaction
                sigma_coll_patch = 0.5 * (partsys.sigma_colloid + partsys.sigma_patch)
                f.write(f"pair_coeff {partsys.colloidType} {partsys.patchType} {partsys.par['eps']} {sigma_coll_patch} {minPositionLJ(sigma_coll_patch)}\n")   #! WCA
        ...
        #Bond between beads
        f.write(f"bond_coeff 1 30.0 {1.5 * partsys.sigma_bead} {partsys.par['eps']} {partsys.sigma_bead}\n") 

        if partsys.par['npatch'] > 0:
            #Bond between bead and patch
            sigma_bead_patch = 0.5 * (partsys.sigma_bead + partsys.sigma_patch)
            f.write(f"bond_coeff 2 30.0 {1.5 * sigma_bead_patch} {partsys.par['eps']} {sigma_bead_patch}\n")

            #Bond for the crosslink between patches
            f.write(f"bond_coeff 3 30.0 {1.5 * partsys.sigma_patch} {partsys.par['eps']} {partsys.sigma_patch}\n")
        ...
        f.write(f"fix 2 all langevin 1.0 1.0 1.0 {partsys.rng.integers(1, 10**8)}\n")      #Langevin thermostat (Tstart Tfin dampening seed)

        if partsys.par['press/berendsen'] > 0:
            f.write(f"fix 3 all press/berendsen iso {partsys.par['press_in']} {partsys.par['press_fin']} 1000.0\n") #If NPT with Berendsen barostat
        ...
        f.write(f"restart {int(partsys.par["restartSteps"])} restarts/polymers.restart restarts/polymers2.restart\n")  #LAMMPS restart files
        f.write("\n")
        f.write(f"dump config all custom {int(partsys.par['dumpsteps'])} configurations/polymers_{file_number}.lammpstrj id mol type x y z\n")
        f.write("\n")
        ...
        f.write(f"thermo {int(partsys.par['thermosteps'])}\n")
        ...
        if partsys.par['npatch'] > 0 and partsys.par['crosslink'] == True:
            f.write(f"fix createBonds all bond/create 1 {partsys.patchType} {partsys.patchType} {1 * partsys.sigma_bead} 3 iparam 1 2 jparam 1 2\n")

        f.write("minimize 1.0e-4 1.0e-6 100 1000\n") #Little energy minimization to stabilize initial configuration

        f.write(f"run {int(partsys.par['totsteps'])}\n")
